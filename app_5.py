...
# --- 3. 상담 통계 ---
    ...
    if no_show_col and no_show_y.sum() > 0:
        st.warning("노쇼 값이 있습니다. 노쇼를 별도로 다루시는 경우에는 원 데이터를 다시 확인하시기 바랍니다.")
    ...
    # 회기수 전체 범위 산출 (예: 1~N)
    min_sess = session_by_user_month['회기수'].min()
    max_sess = session_by_user_month['회기수'].max()
    all_sessions = list(range(min_sess, max_sess+1))
    ...
    # 합계 행/열 추가
    session_people_table['합계'] = session_people_table.sum(axis=1)
    session_people_table.loc['합계'] = session_people_table.sum()
    st.dataframe(session_people_table)

    # --- 상담회기별 이용 건수 (회) ---
    st.markdown("상담회기별 이용 건수 (회)")

    # 실제 회기 칼럼의 값 기준으로 카운트
    session_count_table = (
        df_counseling
        .dropna(subset=['상담연월', '회기'])
        .groupby(['회기', '상담연월'])['사례번호']
        .count()
        .unstack(fill_value=0)
        .reindex(all_sessions, fill_value=0)
        .sort_index()
    )
    session_count_table['합계'] = session_count_table.sum(axis=1)
    session_count_table.loc['합계'] = session_count_table.sum()
    ...
    excel_tables = {
        ...
        "상담회기별_이용인원": session_people_table,
